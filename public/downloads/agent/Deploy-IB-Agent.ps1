# Deploy-IB-Agent.ps1
# Interbrand. No wizard. Administrator on that customer host.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\Deploy-IB-Agent.ps1 -AgentSecret '<IOPS secret>'
#
param(
...
Invoke-WebRequest -UseBasicParsing -TimeoutSec 60 `
  ...
  -OutFile $deploy
& $deploy -CustomerCode 'IB' -RoleTags 'sql' -AgentSecret $AgentSecret -AppHttpsUrl $AppHttpsUrl
