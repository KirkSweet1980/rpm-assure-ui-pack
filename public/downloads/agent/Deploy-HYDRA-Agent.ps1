# Deploy-HYDRA-Agent.ps1
# Hydrasales. No wizard. Administrator on that customer host.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\Deploy-HYDRA-Agent.ps1 -AgentSecret '<IOPS secret>'
#
param(
...
Invoke-WebRequest -UseBasicParsing -TimeoutSec 60 `
  ...
  -OutFile $deploy
& $deploy -CustomerCode 'HYDRA' -RoleTags 'syspro' -AgentSecret $AgentSecret -AppHttpsUrl $AppHttpsUrl
