# Deploy-RSR-Agent.ps1
# Redsun Raisins. No wizard. Administrator on that customer host.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\Deploy-RSR-Agent.ps1 -AgentSecret '<IOPS secret>'
#
param(
...
Invoke-WebRequest -UseBasicParsing -TimeoutSec 60 `
  ...
  -OutFile $deploy
& $deploy -CustomerCode 'RSR' -RoleTags 'syspro' -AgentSecret $AgentSecret -AppHttpsUrl $AppHttpsUrl
