# Deploy-MEDIPOS-Agent.ps1
# MEDiPOS Medical Scheme. No wizard. Administrator on that customer host.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\Deploy-MEDIPOS-Agent.ps1 -AgentSecret '<IOPS secret>'
#
param(
...
Invoke-WebRequest -UseBasicParsing -TimeoutSec 60 `
  ...
  -OutFile $deploy
& $deploy -CustomerCode 'MEDIPOS' -RoleTags 'sql' -AgentSecret $AgentSecret -AppHttpsUrl $AppHttpsUrl
