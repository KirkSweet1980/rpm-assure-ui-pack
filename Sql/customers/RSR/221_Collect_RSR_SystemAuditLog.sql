...
PRINT CONCAT(N'=== RSR SystemAuditLog ', CONVERT(char(10), @SnapshotDate, 23), N' ===');

IF NOT EXISTS (
  SELECT 1 FROM [RPM_CENTRAL].[RPMAssure_App].dbo.Dim_Customer
  WHERE CustomerCode = @CustomerCode AND Active = 1)
BEGIN
  PRINT N'RSR not active on central.'; RETURN;
END
...
PRINT N'=== Done RSR SystemAuditLog ===';
GO
