# Deploy-SBS-Agent.ps1
# Simply Bright. No wizard. Administrator on that customer host.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\Deploy-SBS-Agent.ps1 -AgentSecret '<IOPS secret>'
#
param(
...
Invoke-WebRequest -UseBasicParsing -TimeoutSec 60 `
  ...
  -OutFile $deploy
& $deploy -CustomerCode 'SBS' -RoleTags 'sql' -AgentSecret $AgentSecret -AppHttpsUrl $AppHttpsUrl
