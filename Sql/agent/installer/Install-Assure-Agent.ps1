...
function Unlock-RpmaAgentFolder([string]$AgentRoot) {
  # Previous installs may have locked Agent.Secrets.bin to SYSTEM-only.
  W "Unlocking agent folder for secrets write..."
  ...
  try {
    & icacls $AgentRoot /grant "Administrators:(OI)(CI)F" /T /C /Q 2>$null | Out-Null
    & icacls $AgentRoot /grant "SYSTEM:(OI)(CI)F" /T /C /Q 2>$null | Out-Null
  } catch {}
  $sec = Join-Path $AgentRoot 'Agent.Secrets.bin'
  if (Test-Path -LiteralPath $sec) {
    try {
      & icacls $sec /grant "Administrators:F" /C /Q 2>$null | Out-Null
      attrib -R -S -H $sec 2>$null
      Remove-Item -LiteralPath $sec -Force -EA SilentlyContinue
      W "Removed old Agent.Secrets.bin"
    } catch {
      W ("WARN could not remove Agent.Secrets.bin: " + $_.Exception.Message)
    }
  }
...
# Enable cover - pass 0/1 strings (safe for powershell -File)
try {
  if ($local.Syspro -or $local.Pulseway -or $local.Bitdefender -or $local.Cove) {
    ...
    if (Test-Path $enablePs1) {
      ...
      $flagSys = if ($local.Syspro) { '1' } else { '0' }
      $flagRmm = if ($local.Pulseway) { '1' } else { '0' }
      $flagEpp = if ($local.Bitdefender) { '1' } else { '0' }
      $flagCove = if ($local.Cove) { '1' } else { '0' }
...
Unlock-RpmaAgentFolder $agentRoot
...
"@
  ...
  $custBody = @"
# Generated by Assure Agent Wizard. No local SQL / SYSPRO on this host.
`$CustomerCode = '$CustomerCode'
`$DisplayName = '$($DisplayName.Replace("'", "''"))'
