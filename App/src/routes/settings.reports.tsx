...
function ReportSchedulesPage() {
  ...
  async function onRun(slot: "daily" | "weekly" | "monthly") {
    ...
    try {
      ...
      setMsg(
        `${slot}: wrote ${r.written ?? 0}, mailed ${r.sent}` +
          (r.folder ? ` · ${r.folder}` : "") +
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rpma-panel space-y-3 p-4">
        ...
        {!smtpOn ? (
          <p className="text-[12px] text-muted">
            SMTP is off — packs still write to{" "}
            <span className="font-mono">C:\RPM-Assure\App\data\packs</span>. Turn email on under{" "}
            <Link to="/settings/smtp" className="underline">
              Email
            </Link>{" "}
            to mail them as well.
          </p>
        ) : null}
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          <Toggle label="Weekly patch compliance packs" checked={sched.patchPackWeekly !== false} onChange={(v) => setSched((s) => ({ ...s, patchPackWeekly: v }))} />
          <Toggle label="Weekly full assurance packs" checked={sched.fullPackWeekly !== false} onChange={(v) => setSched((s) => ({ ...s, fullPackWeekly: v }))} />
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button size="sm" variant="secondary" disabled={busy} onClick={() => void onRun("daily")}>
            <Send className="size-3.5" /> Generate daily now
          </Button>
