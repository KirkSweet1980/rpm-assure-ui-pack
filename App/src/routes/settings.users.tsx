    ...
    <div className="space-y-6">
      ...
      <section className="rpma-panel overflow-hidden p-0">
        <div className="flex items-end justify-between px-4 py-3">
          <h2 className="text-[16px] font-extrabold text-fg">Create Account</h2>
          <Button type="button" size="sm" disabled={busy} onClick={() => void createUser()}>
          ...
          <table className="rpma-xls">
            ...
            <tbody>
              <tr>
                <td>Email</td>
                <td>
                  <input className="w-full border-0 bg-transparent text-[12px] outline-none" type="email" value={draft.email} onChange={(e) => setDraft((d) => ({ ...d, email: e.target.value }))} />
                </td>
              </tr>
              <tr>
                <td>Display Name</td>
                <td>
                  <input className="w-full border-0 bg-transparent text-[12px] outline-none" value={draft.displayName} onChange={(e) => setDraft((d) => ({ ...d, displayName: e.target.value }))} />
                </td>
              </tr>
              <tr>
                <td>Temporary Password</td>
                <td>
                  <input className="w-full border-0 bg-transparent text-[12px] outline-none" type="password" value={draft.password} onChange={(e) => setDraft((d) => ({ ...d, password: e.target.value }))} />
                </td>
              </tr>
              <tr>
                <td>Role</td>
                <td>
                  <select className="border-0 bg-transparent text-[12px] outline-none" value={draft.staffRole} onChange={(e) => setDraft((d) => ({ ...d, staffRole: e.target.value as StaffRole }))}>
                    {STAFF_ROLES.map((r) => (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {customers.map((c) => (
                        <label key={c.code} className="inline-flex items-center gap-1 text-[11px]">
                          <input type="checkbox" checked={draft.customerCodes.includes(c.code)} onChange={() => toggleDraftCode(c.code)} />
                          {c.name}
                        </label>
                      ))}
                    </div>
      ...
      {users.some((u) => u.staffRole !== "PlatformAdmin") && customers.length > 0 ? (
        <section className="rpma-panel overflow-hidden p-0">
          <div className="px-4 py-3">
            <h2 className="text-[16px] font-extrabold text-fg">Customer Scope</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="rpma-xls">
              <thead>
