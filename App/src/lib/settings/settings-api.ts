...
function emptyCustomerDetail(row: import("@/lib/data/types").PortfolioRow): import("@/lib/data/types").CustomerDetailPayload {
  return {
    ...
    operationalAssurance: {
      ...
      scorePct: row.healthRag === "Green" ? 88 : row.healthRag === "Amber" ? 68 : row.healthRag === "Off" ? null : 42,
...
export const runAllApiSync = createServerFn({ method: "POST" }).handler(async () => {
  return startApiFeedSyncHandler();
});

export type ApiFeedLeg = {
  name: string;
  label: string;
  kind: string;
  status: string;
...
function emptyFeedStatus(message: string): ApiFeedSyncStatus {
  return {
    ok: true,
    running: false,
    pct: 0,
...
function parseFeedJson(raw: string): ApiFeedSyncStatus | null {
  ...
  try {
    ...
    if (running && started) {
      ...
      if (Number.isFinite(age) && age > 25 * 60 * 1000) running = false;
    }
    return {
      ok: true,
      running,
      pct: Number(j.pct ?? 0) || 0,
      ...
      legs: legsRaw.map((rawLeg) => {
        ...
        return {
          ...
          startedUtc: typeof r.startedUtc === "string" ? r.startedUtc : null,
          finishedUtc: typeof r.finishedUtc === "string" ? r.finishedUtc : null,
        };
      }),
    };
  } catch {
...
async function startApiFeedSyncHandler() {
  ...
  const path = await import("node:path");
  const { spawn } = await import("node:child_process");
  const current = await readApiFeedStatusFile();
  if (current.running) {
    return { ok: true as const, started: false as const, message: "API collect already running.", status: current };
  }
  const candidates = [
  ...
  if (!script) {
    return {
      ...
      message: "Runner missing. Update the app server so Sql\\ops\\Run-All-Api-Collects-Scheduled.ps1 is copied.",
      status: current,
    };
  }
  try {
    ...
    fs.writeFileSync(apiStatusPath(), JSON.stringify(seed, null, 2), "utf8");
  } catch {
    /* status write optional */
  }
  try {
    const child = spawn(
      "powershell.exe",
      ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", script, "-Root", "C:\\RPM-Assure"],
...
export const fetchApiFeedSyncStatus = createServerFn({ method: "GET" }).handler(async () => {
  return readApiFeedStatusFile();
});

export const startApiFeedSync = createServerFn({ method: "POST" }).handler(async () => {
  return startApiFeedSyncHandler();
});
