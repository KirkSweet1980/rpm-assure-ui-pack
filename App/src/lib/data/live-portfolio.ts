...
SELECT TOP 400
  ...
  IsMajor, ExternalRef, OwnerName, SourceSystem, BusinessImpact, ModuleCode,
...
ORDER BY
  ...
  if (want("ams") || want("shell")) {
    ...
    incidents = ((fi?.recordset ?? []) as Array<Record<string, unknown>>).map((r) => ({
      ...
      moduleCode: r.ModuleCode != null ? String(r.ModuleCode) : null,
...
ORDER BY
  ...
  TimeCreatedUtc DESC`);
      const evRows = (evRes.recordset ?? []) as Array<{
        HostName?: string;
        TimeCreatedUtc?: Date | string | null;
        LogName?: string;
        EventId?: number;
        LevelName?: string;
        ProviderName?: string;
      ...
      const windowsEvents = evRows.map((r) => ({
        hostName: String(r.HostName ?? ""),
        timeCreatedUtc: toIso(r.TimeCreatedUtc ?? null),
        logName: String(r.LogName ?? ""),
        eventId: Number(r.EventId ?? 0),
        levelName: String(r.LevelName ?? "Error"),
...
ORDER BY
  ...
  RaisedAt DESC`);
      ...
      for (const ev of windowsEvents) {
        ...
        rmm.alerts.push({
          notificationId: `evt-${ev.hostName}-${ev.timeCreatedUtc}-${ev.eventId}-${ev.logName}`,
          deviceId: null,
          deviceName: ev.hostName || "Agent host",
          severity: crit ? "Critical" : "Error",
          title: `${ev.providerName || ev.logName} ${ev.eventId}`.trim(),
          message: [ev.logName, ev.message].filter(Boolean).join(" — "),
          raisedAt: ev.timeCreatedUtc,
          isActive: true,
          source: "agent",
        });
      }
      if (rmm.summary && windowsEvents.length) {
        const withAlerts = new Set(
          rmm.alerts.map((a) => (a.deviceId || a.deviceName || "").toLowerCase()),
        );
        rmm.summary.devicesWithAlerts = withAlerts.size;
        const critN = windowsEvents.filter((e) => e.levelName.toLowerCase() === "critical").length;
        if (rmm.summary.criticalAlerts != null) rmm.summary.criticalAlerts += critN;
      }
    } catch {
      rmm.windowsEvents = [];
    }
