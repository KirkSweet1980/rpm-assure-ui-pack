...
export function buildCustomPackHtml(opts: {
  ...
  if (fieldOn(selected, "rmm_fleet") || fieldOn(selected, "rmm_alerts") || fieldOn(selected, "rmm_patches") || fieldOn(selected, "rmm_disks") || fieldOn(selected, "rmm_reboot")) {
    if (!rmm?.enabled && !rs) {
      ...
      if (fieldOn(selected, "rmm_patches") && rs) {
        rows.push(["Outstanding patches (estate)", esc(String(rs.patchMissing ?? "—"))]);
        rows.push(["Devices reporting patches", esc(String(rs.patchDevicesReporting ?? "—"))]);
        rows.push(["Pending", esc(String(rs.patchPending ?? "—"))]);
      }
  ...
  if (fieldOn(selected, "rmm_patch_table") && rmm?.devices?.length) {
    const devices = [...rmm.devices].sort(
      (a, b) => (b.patchMissing ?? -1) - (a.patchMissing ?? -1),
    );
    const servers = devices.filter((d) => isRmmServer(d)).slice(0, 40);
    const ws = devices.filter((d) => isRmmWorkstation(d)).slice(0, 40);
    const rowHtml = (list: typeof devices) =>
      list.length
        ? list
            .map(
              (d) =>
                `<tr><td>${esc(d.name || d.deviceId)}</td><td>${esc(isRmmServer(d) ? "Server" : "Workstation")}</td><td>${d.isOnline === false ? "Offline" : d.isOnline ? "Online" : "—"}</td><td style="text-align:right" class="${(d.patchMissing ?? 0) > 0 ? "bad" : "ok"}">${esc(d.patchMissing ?? "—")}</td><td style="text-align:right">${esc(d.patchPending ?? "—")}</td></tr>`,
            )
            .join("")
        : `<tr><td colspan="5" class="muted">None</td></tr>`;
    sections.push(`<h2 class="sec">Outstanding patches by device</h2>
    <h3 class="sub">Servers</h3>
    <table class="ams"><thead><tr><th class="dark">Device</th><th class="dark">Class</th><th class="dark">Online</th><th class="dark">Outstanding</th><th class="dark">Pending</th></tr></thead>
    <tbody>${rowHtml(servers)}</tbody></table>
    <h3 class="sub">Workstations</h3>
    <table class="ams"><thead><tr><th class="dark">Device</th><th class="dark">Class</th><th class="dark">Online</th><th class="dark">Outstanding</th><th class="dark">Pending</th></tr></thead>
    <tbody>${rowHtml(ws)}</tbody></table>`);
  }
