...
export function EcoBoard({ data }: { data: CustomerDetailPayload }) {
  ...
  const rmmOn = customer.pulsewayOnlineCount ?? customer.pulsewayServerOnline ?? 0;
  const rmmOff = customer.pulsewayOfflineCount ?? customer.pulsewayServerOffline ?? 0;
  const rmmPie = [
      ...
      <div className="rpma-eco-board">
        ...
        <Pane title="RPM RMM" tip="Pulseway agents on the latest snapshot." {...wgt("rmm")}>
          {isPillarCovered(cover, "rmm") ? (
            <div className="grid grid-cols-2 gap-2">
              ...
              <StatCard
                label="Critical"
                value={customer.pulsewayCriticalAlerts ?? 0}
                tone={(customer.pulsewayCriticalAlerts ?? 0) > 0 ? "red" : "green"}
              />
              <StatCard
                label="Servers"
                value={`${customer.pulsewayServerOnline ?? 0}/${(customer.pulsewayServerOnline ?? 0) + (customer.pulsewayServerOffline ?? 0)}`}
              />
            </div>
          ) : (
        ...
        <Pane title="Endpoint Security" tip="Bitdefender / RPM EPP." {...wgt("epp")}>
          {isPillarCovered(cover, "epp") ? (
            <div className="grid grid-cols-2 gap-2">
              <StatCard label="Endpoints" value={customer.eppDeviceCount ?? 0} />
              <StatCard
                label="Infected"
                value={customer.bdInfectedCount ?? 0}
                tone={(customer.bdInfectedCount ?? 0) > 0 ? "red" : "green"}
              />
            </div>
          ) : (
