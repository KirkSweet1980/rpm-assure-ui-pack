# PASTE THIS WHOLE BLOCK into Administrator PowerShell on the APP server
...
function Pick-Map($title,$rows,$default){
  ...
  $i=1; foreach($r in $rows){ Write-Host ("  {0}) {1}" -f $i,$r); $i++ }
  $pick=Ask $title $default
  if($pick -match '^(NONE|none|n)$'){ return @() }
  if($pick -match '^(ALL|all)$'){ return 1..$rows.Count }
  return @($pick.Split(',') | ForEach-Object { [int]($_.Trim()) })
}
...
$rr=@(Rows $cand.StdOut)
$ix=Pick-Map 'Map which Pulseway orgs (comma / ALL / NONE)' $rr $(if($rr.Count -eq 1){'1'}else{'NONE'})
if($ix.Count -gt 0){
  foreach($n in $ix){
    if($n -lt 1 -or $n -gt $rr.Count){continue}
    $org=Esc (($rr[$n-1] -split '\|')[0].Trim())
    Sql @"
SET NOCOUNT ON;
...
IF COL_LENGTH(N'dbo.Cove_DeviceStatistics',N'PartnerName') IS NOT NULL
  SELECT TOP 25 PartnerName, COUNT_BIG(*) Devices FROM dbo.Cove_DeviceStatistics WITH (NOLOCK)
  WHERE SnapshotDate=(SELECT MAX(SnapshotDate) FROM dbo.Cove_DeviceStatistics WITH (NOLOCK)) AND ($(LikeOr 'PartnerName' $tok))
  GROUP BY PartnerName ORDER BY Devices DESC;
ELSE IF COL_LENGTH(N'dbo.Cove_DeviceStatistics',N'Product') IS NOT NULL
  SELECT TOP 25 Product AS PartnerName, COUNT_BIG(*) Devices FROM dbo.Cove_DeviceStatistics WITH (NOLOCK)
  WHERE SnapshotDate=(SELECT MAX(SnapshotDate) FROM dbo.Cove_DeviceStatistics WITH (NOLOCK)) AND ($(LikeOr 'Product' $tok))
  GROUP BY Product ORDER BY Devices DESC;
ELSE SELECT N'NO_TABLE';
"@
Write-Host 'Candidates:'; Write-Host $cove.StdOut
$cr=@(Rows $cove.StdOut)
$ix=Pick-Map 'Map which Cove partners (comma / ALL / NONE)' $cr $(if($cr.Count -eq 1){'1'}else{'NONE'})
if($ix.Count -gt 0){
  foreach($n in $ix){
    if($n -lt 1 -or $n -gt $cr.Count){continue}
    $pn=Esc (($cr[$n-1] -split '\|')[0].Trim())
    Sql @"
SET NOCOUNT ON;
...
IF COL_LENGTH(N'dbo.Cove_DeviceStatistics',N'PartnerName') IS NOT NULL
  UPDATE dbo.Cove_DeviceStatistics SET CustomerCode=N'$codeLit' WHERE PartnerName=N'$pn';
...
IF COL_LENGTH(N'dbo.Bitdefender_Endpoints',N'CompanyName') IS NOT NULL
  SELECT TOP 25 CompanyName, COUNT_BIG(*) Endpoints FROM dbo.Bitdefender_Endpoints WITH (NOLOCK)
  WHERE SnapshotDate=(SELECT MAX(SnapshotDate) FROM dbo.Bitdefender_Endpoints WITH (NOLOCK)) AND ($(LikeOr 'CompanyName' $tok))
  GROUP BY CompanyName ORDER BY Endpoints DESC;
ELSE IF COL_LENGTH(N'dbo.Bitdefender_Endpoints',N'Company') IS NOT NULL
  SELECT TOP 25 Company AS CompanyName, COUNT_BIG(*) Endpoints FROM dbo.Bitdefender_Endpoints WITH (NOLOCK)
  WHERE SnapshotDate=(SELECT MAX(SnapshotDate) FROM dbo.Bitdefender_Endpoints WITH (NOLOCK)) AND ($(LikeOr 'Company' $tok))
  GROUP BY Company ORDER BY Endpoints DESC;
ELSE SELECT N'NO_TABLE';
"@
Write-Host 'Candidates:'; Write-Host $bd.StdOut
$br=@(Rows $bd.StdOut)
$ix=Pick-Map 'Map which BD companies (comma / ALL / NONE)' $br $(if($br.Count -eq 1){'1'}else{'NONE'})
if($ix.Count -gt 0){
  foreach($n in $ix){
    if($n -lt 1 -or $n -gt $br.Count){continue}
    $cn=Esc (($br[$n-1] -split '\|')[0].Trim())
    Sql @"
SET NOCOUNT ON;
...
'@ | Set-Content -LiteralPath (Join-Path $dest 'Complete-Customer-Cover.ps1') -Encoding ASCII
Unblock-File (Join-Path $dest 'Complete-Customer-Cover.ps1')
Write-Host 'WROTE Complete-Customer-Cover.ps1' -ForegroundColor Green
powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $dest 'Complete-Customer-Cover.ps1') -CustomerCode SIRF
