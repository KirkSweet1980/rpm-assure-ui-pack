...
function Test-RpmaCentralViaHttps([string]$Server) {
  if (-not $Server) { return $false }
  if ($Server -eq '.' -or $Server -eq '(local)' -or $Server -match '^(localhost|127\.|::1)') { return $false }
  if ($Server -match '14333|102\.222\.21\.220|https-only|assure\.rpmresources|rpmwinrm') { return $true }
  if (Get-Variable -Name PreferHttps -ErrorAction SilentlyContinue) {
    if ($PreferHttps) { return $true }
  }
  return $false
}

function Invoke-RpmaHttpsSql {
  param([string]$SqlText, [switch]$Tsv)
  $lib = 'C:\RPM-Assure\Agent\Lib-RpmaHttps.ps1'
  if (-not (Get-Command Invoke-RpmaAssureHttps -ErrorAction SilentlyContinue)) {
    if (Test-Path -LiteralPath $lib) { . $lib }
  }
  if (-not (Get-Command Invoke-RpmaAssureHttps -ErrorAction SilentlyContinue)) {
    return [pscustomobject]@{ ExitCode = 1; Text = 'HTTPS SQL helper missing (Lib-RpmaHttps.ps1)' }
  }
  $code = ''
  if (Get-Variable -Name CustomerCode -ErrorAction SilentlyContinue) { $code = [string]$CustomerCode }
  try {
    $r = Invoke-RpmaAssureHttps -Path '/api/agent/sql' -Method POST -TimeoutSec 180 -Body @{
      kind         = 'sql'
      sql          = $SqlText
      tsv          = [bool]$Tsv
      customerCode = $code
    ...
    if ($r.Json -and $r.Json.ok) {
      return [pscustomobject]@{ ExitCode = 0; Text = [string]$r.Json.text }
    }
    $msg = if ($r.Json -and $r.Json.error) { [string]$r.Json.error } else { [string]$r.Text }
    return [pscustomobject]@{ ExitCode = 1; Text = $msg }
  } catch {
...
function Invoke-RpmaSql {
  ...
  if (Test-RpmaCentralViaHttps $Server) {
    Write-RpmaLog ('HTTPS SQL ' + $Server.Substring(0, [Math]::Min(24, $Server.Length)))
    return Invoke-RpmaHttpsSql -SqlText $SqlText -Tsv:$Tsv
  }
