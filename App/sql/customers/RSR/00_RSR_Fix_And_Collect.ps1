...
$ec = Run-Sql '102.222.21.220,14333' 'rpmassure' '' (Join-Path $dir '301_Central_Register_RSR.sql') 'RPMAssure_App'
...
if ($ec -ne 0) {
  ...
  if ($ec2 -ne 0) {
    ...
    Run-Sql '.' 'rpmassure' '' (Join-Path $dir '303b_Map_Only_rpmassure.sql') | Out-Null
...
& sqlcmd -S '.' -U 'rpmassure' -P '' -C -b -Q "SELECT TOP 3 CustomerCode, DisplayName FROM [RPM_CENTRAL].[RPMAssure_App].dbo.Dim_Customer"
