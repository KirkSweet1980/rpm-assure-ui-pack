/*
  RSS — FinSight DTR Levels 1+2+3 (all company DBs) → central Syspro_Dtr*
  Run ON the customer SYSPRO SQL host (linked server RPM_CENTRAL).

  powershell -NoProfile -ExecutionPolicy Bypass -File C:\RPM-Assure\Sql\collect\Run-Dtr-AllLevels.ps1 -CustomerCode RSS
*/
SET NOCOUNT ON;
SET XACT_ABORT OFF;
...
PRINT CONCAT(N'=== RSS DTR L1+L2+L3 ', CONVERT(char(10), @SnapshotDate, 23), N' ===');
...
PRINT CONCAT(N'=== RSS DTR L1-3 total rows=', @total, N' ===');
