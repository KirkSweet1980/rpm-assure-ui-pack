...
import { coveHealthFor, finalizeEstateHealth, healthFor, healthScorePctFromRag, ragToAvailabilityPct, ragToScorePct, rmmHealthFor } from "./health-rag";
...
ORDER BY UserPrincipalName, DisplayName`);
  ...
  try {
    ...
    // Derived SLA/availability only from covered estate health — never invent SYSPRO SLA when SYSPRO is No cover
    if (availabilitySla && (availabilitySla.source === "derived" || availabilitySla.source === "stub")) {
      if (!anyCover(cover)) {
        ...
        // RMM availability from servers only (exclude workstations)
        ...
        if (cover.rmm) {
          ...
          const sn = so + sf;
          if (sn > 0) {
            avail = Math.round((so / sn) * 1000) / 10;
          }
        }
        const legs: string[] = [];
