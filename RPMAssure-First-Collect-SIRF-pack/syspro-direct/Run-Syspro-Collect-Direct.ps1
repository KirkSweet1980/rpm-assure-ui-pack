...
"@
  ...
  # Build number - often only in LicenseXml (not a discrete column)
  $bd = ''
  if ($xml) {
    if ($xml -match '(?i)<BuildNumber[^>]*>([^<]+)') { $bd = $Matches[1].Trim() }
    elseif ($xml -match '(?i)<ProductBuild[^>]*>([^<]+)') { $bd = $Matches[1].Trim() }
    elseif ($xml -match '(?i)<Build[^>]*>([^<]+)') { $bd = $Matches[1].Trim() }
    elseif ($xml -match '(?i)BuildNumber\s*=\s*"([^"]+)"') { $bd = $Matches[1].Trim() }
  }
  # ProductVersion like 8.0.0.123 -> last segment as build when no explicit build
  if (-not $bd -and $pv -match '^\d+(\.\d+){2,}\.(\d+)$') {
    $bd = $Matches[2]
  }
  # Also try SYSPRO style "8.0.34" mid segment
  if (-not $bd -and $pv -match '^\d+\.\d+\.(\d+)$') {
    $bd = $Matches[1]
  }
  ...
  $delV = "IF OBJECT_ID(N'dbo.Syspro_VersionInfo',N'U') IS NOT NULL DELETE FROM dbo.Syspro_VersionInfo WHERE SnapshotDate='$snap' AND InstanceName=$instLit;"
  $insV = "IF OBJECT_ID(N'dbo.Syspro_VersionInfo',N'U') IS NOT NULL INSERT INTO dbo.Syspro_VersionInfo (SnapshotDate,InstanceName,ProductName,ProductVersion,BuildNumber,LicenseType,Users,CompanyCount,LicenseExpiry,CustomerName,ImportDate,ServerName,ImportedAt) VALUES ('$snap',$instLit," + (ConvertTo-RpmaSqlLit $pn) + "," + (ConvertTo-RpmaSqlLit $pv) + "," + (ConvertTo-RpmaSqlLit $bd) + ",$lt,$us,$cc,$ex," + (ConvertTo-RpmaSqlLit $cn) + ",$id," + (ConvertTo-RpmaSqlLit $InstanceName) + ",SYSUTCDATETIME());"
  $rv = Invoke-RpmaSql -Server $CentralDataSource -User $CentralSqlUser -Pass $CentralSqlPassword -Database $CentralDatabase -SqlText ("SET NOCOUNT ON;`r`n" + $delV + "`r`n" + $insV)
