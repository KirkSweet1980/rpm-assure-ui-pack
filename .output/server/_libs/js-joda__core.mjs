...
//#region node_modules/@js-joda/core/dist/js-joda.esm.js
var js_joda_esm_exports = /* @__PURE__ */ __exportAll({
...
function createErrorType(name, init, superErrorClass) {
	...
	function JsJodaException(message) {
		if (!Error.captureStackTrace) this.stack = (/* @__PURE__ */ new Error()).stack;
...
function assert(assertion, msg, error) {
	if (!assertion) if (error) throw new error(msg);
...
var init_js_joda_esm = __esmMin((() => {
	...
	Duration = function(_TemporalAmount) {
		function Duration(seconds, nanos) {
			var _this = _TemporalAmount.call(this) || this;
		...
		Duration.ofMillis = function ofMillis(millis) {
			...
			return Duration._create(secs, mos * 1e6);
		};
		...
		Duration.parse = function parse(text) {
			...
			var matches = (/* @__PURE__ */ new RegExp("([-+]?)P(?:([-+]?[0-9]+)D)?(T(?:([-+]?[0-9]+)H)?(?:([-+]?[0-9]+)M)?(?:([-+]?[0-9]+)(?:[.,]([0-9]{0,9}))?S)?)?", "i")).exec(text);
			if (matches !== null) {
		...
		_proto.plusAmountUnit = function plusAmountUnit(amountToAdd, unit) {
			...
			if (unit instanceof ChronoUnit) {
				switch (unit) {
					...
					case ChronoUnit.MICROS: return this.plusSecondsNanos(MathUtil.intDiv(amountToAdd, 1e9) * 1e3, MathUtil.intMod(amountToAdd, 1e9) * 1e3);
		...
		_proto.toString = function toString() {
			...
			if (secs === 0 && fractionNanos === 0 && rval.length > 2) return rval;
			if (secs === 0 && negativeFraction) rval += "-0";
			else rval += "" + secs;
			if (fractionNanos > 0) {
				var fraction = "" + fractionNanos;
				while (fraction.length < 9) fraction = "0" + fraction;
				while (fraction.charAt(fraction.length - 1) === "0") fraction = fraction.slice(0, fraction.length - 1);
				rval += "." + fraction;
			}
			rval += "S";
			return rval;
	...
	YearConstants = function YearConstants() {};
	ChronoUnit = function(_TemporalUnit) {
		function ChronoUnit(name, estimatedDuration) {
			var _this = _TemporalUnit.call(this) || this;
			_this._name = name;
	...
	ChronoField = function(_TemporalField) {
		function ChronoField(name, baseUnit, rangeUnit, range) {
			var _this = _TemporalField.call(this) || this;
			_this._name = name;
	...
	DayOfWeek = function(_TemporalAccessor) {
		function DayOfWeek(ordinal, name) {
			var _this = _TemporalAccessor.call(this) || this;
		...
		DayOfWeek.valueOf = function valueOf(name) {
			var ordinal = 0;
			for (; ordinal < ENUMS.length; ordinal++) if (ENUMS[ordinal].name() === name) break;
	...
	Month = function(_TemporalAccessor) {
		...
		_proto.length = function length(leapYear) {
			switch (this) {
				case Month.FEBRUARY: return leapYear ? 29 : 28;
				case Month.APRIL:
				case Month.JUNE:
		...
		_proto.firstDayOfYear = function firstDayOfYear(leapYear) {
			...
			switch (this) {
				...
				case Month.FEBRUARY: return 32;
				case Month.MARCH: return 60 + leap;
				case Month.APRIL: return 91 + leap;
				case Month.MAY: return 121 + leap;
				case Month.JUNE: return 152 + leap;
				case Month.JULY: return 182 + leap;
				case Month.AUGUST: return 213 + leap;
				case Month.SEPTEMBER: return 244 + leap;
				case Month.OCTOBER: return 274 + leap;
				case Month.NOVEMBER: return 305 + leap;
				case Month.DECEMBER:
		...
		_proto.firstMonthOfQuarter = function firstMonthOfQuarter() {
			switch (this) {
				case Month.JANUARY:
				case Month.FEBRUARY:
				case Month.MARCH: return Month.JANUARY;
				case Month.APRIL:
				case Month.MAY:
				case Month.JUNE: return Month.APRIL;
				case Month.JULY:
				case Month.AUGUST:
				case Month.SEPTEMBER: return Month.JULY;
				case Month.OCTOBER:
				case Month.NOVEMBER:
				case Month.DECEMBER:
				default: return Month.OCTOBER;
			}
		};
		_proto.query = function query(_query) {
		...
		_proto.toString = function toString() {
			switch (this) {
				...
				default: return "unknown Month, value: " + this.value();
			}
		};
		_proto.toJSON = function toJSON() {
			return this.toString();
		...
		Month.valueOf = function valueOf(name) {
			var ordinal = 0;
			for (; ordinal < MONTHS.length; ordinal++) if (MONTHS[ordinal].name() === name) break;
	...
	Period = function(_TemporalAmount) {
		...
		_proto.addTo = function addTo(temporal) {
			requireNonNull(temporal, "temporal");
			if (this._years !== 0) if (this._months !== 0) temporal = temporal.plus(this.toTotalMonths(), ChronoUnit.MONTHS);
		...
		_proto.subtractFrom = function subtractFrom(temporal) {
			requireNonNull(temporal, "temporal");
			if (this._years !== 0) if (this._months !== 0) temporal = temporal.minus(this.toTotalMonths(), ChronoUnit.MONTHS);
	...
	Fixed = function(_ZoneRules) {
		function Fixed(offset) {
			var _this = _ZoneRules.call(this) || this;
	...
	ZoneOffset = function(_ZoneId) {
		function ZoneOffset(totalSeconds) {
			var _this = _ZoneId.call(this) || this;
		...
		ZoneOffset.of = function of(offsetId) {
			...
			switch (offsetId.length) {
				case 2: offsetId = offsetId[0] + "0" + offsetId[1];
				case 3:
				...
				case 9:
					...
					break;
				default: throw new DateTimeException("Invalid ID for ZoneOffset, invalid format: " + offsetId);
			}
	...
	DateTimeBuilder = function(_TemporalAccessor) {
		...
		_proto._mergeTime = function _mergeTime(resolverStyle) {
			if (this.fieldValues.containsKey(ChronoField.CLOCK_HOUR_OF_DAY)) {
				...
				if (resolverStyle !== ResolverStyle.LENIENT) if (resolverStyle === ResolverStyle.SMART && ch === 0);
				else ChronoField.CLOCK_HOUR_OF_DAY.checkValidValue(ch);
				this._addFieldValue(ChronoField.HOUR_OF_DAY, ch === 24 ? 0 : ch);
			}
			if (this.fieldValues.containsKey(ChronoField.CLOCK_HOUR_OF_AMPM)) {
				var _ch = this.fieldValues.remove(ChronoField.CLOCK_HOUR_OF_AMPM);
				if (resolverStyle !== ResolverStyle.LENIENT) if (resolverStyle === ResolverStyle.SMART && _ch === 0);
	...
	DateTimeParseContext = function() {
		function DateTimeParseContext() {
			if (arguments.length === 1) if (arguments[0] instanceof DateTimeParseContext) {
		...
		_proto.subSequenceEquals = function subSequenceEquals(cs1, offset1, cs2, offset2, length) {
			...
			for (var i = 0; i < length; i++) if (cs1[offset1 + i] !== cs2[offset2 + i]) return false;
			return true;
		};
	...
	Parsed = function(_Temporal) {
		function Parsed(dateTimeParseContext) {
			var _this = _Temporal.call(this) || this;
			_this.chrono = null;
			_this.zone = null;
			_this.fieldValues = new EnumMap();
			_this.leapSecond = false;
	...
	Field = function(_TemporalField) {
		...
		Field._getWeek = function _getWeek(date) {
			...
			var firstMonDoy0 = doyThu0 - MathUtil.intDiv(doyThu0, 7) * 7 - 3;
	...
	Unit = function(_TemporalUnit) {
		...
		_proto6.addTo = function addTo(temporal, periodToAdd) {
			switch (this) {
				...
				case QUARTER_YEARS: return temporal.plus(MathUtil.intDiv(periodToAdd, 256), ChronoUnit.YEARS).plus(MathUtil.intMod(periodToAdd, 256) * 3, ChronoUnit.MONTHS);
				default: throw new IllegalStateException("Unreachable");
			}
		};
		_proto6.between = function between(temporal1, temporal2) {
			switch (this) {
				case WEEK_BASED_YEARS: return MathUtil.safeSubtract(temporal2.getLong(WEEK_BASED_YEAR), temporal1.getLong(WEEK_BASED_YEAR));
				case QUARTER_YEARS: return MathUtil.intDiv(temporal1.until(temporal2, ChronoUnit.MONTHS), 3);
				default: throw new IllegalStateException("Unreachable");
			}
		};
	...
	SignStyle = function(_Enum) {
		...
		_proto.parse = function parse(positive, strict, fixedWidth) {
			switch (this) {
				case SignStyle.NORMAL: return !positive || !strict;
				case SignStyle.ALWAYS:
				case SignStyle.EXCEEDS_PAD: return true;
				default: return !strict && !fixedWidth;
			}
		};
		return SignStyle;
	}(Enum);
	...
	TextStyle = function(_Enum) {
		...
		_proto.isStandalone = function isStandalone() {
			switch (this) {
				...
				case TextStyle.NARROW_STANDALONE: return true;
				default: return false;
			}
		};
		_proto.asStandalone = function asStandalone() {
			switch (this) {
				case TextStyle.FULL: return TextStyle.FULL_STANDALONE;
				case TextStyle.SHORT: return TextStyle.SHORT_STANDALONE;
				case TextStyle.NARROW: return TextStyle.NARROW_STANDALONE;
				default: return this;
			}
		};
		_proto.asNormal = function asNormal() {
			switch (this) {
				case TextStyle.FULL_STANDALONE: return TextStyle.FULL;
				case TextStyle.SHORT_STANDALONE: return TextStyle.SHORT;
				case TextStyle.NARROW_STANDALONE: return TextStyle.NARROW;
				default: return this;
			}
		};
		return TextStyle;
	...
	CompositePrinterParser = function() {
		...
		_proto.print = function print(context, buf) {
			...
			try {
				for (var i = 0; i < this._printerParsers.length; i++) if (this._printerParsers[i].print(context, buf) === false) {
		...
		_proto.parse = function parse(context, text, position) {
			if (this._optional) {
				...
				for (var i = 0; i < this._printerParsers.length; i++) {
					pos = this._printerParsers[i].parse(context, text, pos);
					if (pos < 0) {
				...
				for (var _i = 0; _i < this._printerParsers.length; _i++) {
					position = this._printerParsers[_i].parse(context, text, position);
					if (position < 0) break;
				}
				return position;
			}
	...
	FractionPrinterParser = function() {
		...
		_proto.convertToFraction = function convertToFraction(value, zeroDigit) {
			...
			var fraction = "" + MathUtil.intDiv(_value * 1e9, _range);
		...
		_proto.print = function print(context, buf) {
			...
			if (value >= 0) switch (this._signStyle) {
				case SignStyle.EXCEEDS_PAD:
					...
					break;
				case SignStyle.ALWAYS: buf.append(symbols.positiveSign());
			}
				...
				case SignStyle.ALWAYS:
					...
					break;
				case SignStyle.NOT_NEGATIVE: throw new DateTimeException("Field " + this._field + " cannot be printed as the value " + value + " cannot be negative according to the SignStyle");
			}
	...
	ReducedPrinterParser = function(_NumberPrinterParser) {
		function ReducedPrinterParser(field, width, maxWidth, baseValue, baseDate) {
			var _this = _NumberPrinterParser.call(this, field, width, maxWidth, SignStyle.NOT_NEGATIVE) || this;
		...
		_proto2._setValue = function _setValue(context, value, errorPos, successPos) {
			var baseValue = this._baseValue;
			if (this._baseDate != null) baseValue = context.getEffectiveChronology().date(this._baseDate).get(this._field);
		...
		_proto.parse = function parse(context, text, position) {
			switch (this) {
				...
				case SettingsParser.STRICT:
					...
					break;
				case SettingsParser.LENIENT: context.setStrict(false);
			}
			return position;
		};
		_proto.toString = function toString() {
	...
	ZoneIdPrinterParser = function() {
		...
		_proto._parseRegion = function _parseRegion(text, position) {
			...
			if (availableZoneIds.length === 0) return null;
			...
			if (parsedZoneId == null) return null;
			return {
				parsedZoneId,
				parseLength
			};
	...
	DateTimeFormatterBuilder = function() {
		...
		_proto._parsePattern = function _parsePattern(pattern) {
			...
			for (var pos = 0; pos < pattern.length; pos++) {
				...
				if (cur >= "A" && cur <= "Z" || cur >= "a" && cur <= "z") {
					...
					if (field != null) this._parseField(cur, count, field);
					else if (cur === "z") if (count > 4) throw new IllegalArgumentException("Too many pattern letters: " + cur);
						...
						this.appendZoneId();
					} else if (cur === "Z") if (count < 4) this.appendOffset("+HHMM", "+0000");
					...
					else if (cur === "O") if (count === 1) this.appendLocalizedOffset(TextStyle.SHORT);
					else if (count === 4) this.appendLocalizedOffset(TextStyle.FULL);
					...
					var _start = pos++;
					for (; pos < pattern.length; pos++) if (pattern.charAt(pos) === "'") if (pos + 1 < pattern.length && pattern.charAt(pos + 1) === "'") pos++;
					else break;
		...
		_proto._parseField = function _parseField(cur, count, field) {
			switch (cur) {
				...
				case "c":
					switch (count) {
						case 1:
							...
							break;
						case 2: throw new IllegalArgumentException("Invalid number of pattern letters: " + cur);
						case 3:
							this.appendText(field, TextStyle.SHORT_STANDALONE);
							break;
						case 4:
				...
				case "D":
					...
					break;
				default: if (count === 1) this.appendValue(field);
		...
		_proto.appendLiteral = function appendLiteral(literal) {
			assert(literal != null);
			if (literal.length > 0) if (literal.length === 1) this._appendInternalPrinterParser(new CharLiteralPrinterParser(literal.charAt(0)));
		...
		_proto.toFormatter = function toFormatter(resolverStyle) {
			...
			return new DateTimeFormatter(new CompositePrinterParser(this._printerParsers, false), null, DecimalStyle.STANDARD, resolverStyle, null, null, null);
		};
		return DateTimeFormatterBuilder;
	}();
	SECONDS_PER_10000_YEARS = 31556952e4;
	SECONDS_0000_TO_1970 = 62167219200;
	InstantPrinterParser = function() {
		...
		_proto2.print = function print(context, buf) {
			...
			if (inSec >= -SECONDS_0000_TO_1970) {
				...
				if (_hi < 0) if (_ldt.year() === -1e4) buf.replace(pos, pos + 2, "" + (_hi - 1));
			...
			if (this.fractionalDigits === -2) {
				if (inNano !== 0) {
					buf.append(".");
					if (MathUtil.intMod(inNano, 1e6) === 0) buf.append(("" + (MathUtil.intDiv(inNano, 1e6) + 1e3)).substring(1));
					else if (MathUtil.intMod(inNano, 1e3) === 0) buf.append(("" + (MathUtil.intDiv(inNano, 1e3) + 1e6)).substring(1));
					else buf.append(("" + (inNano + 1e9)).substring(1));
				}
		...
		_proto2.parse = function parse(context, text, position) {
			...
			var pos = new DateTimeFormatterBuilder().append(DateTimeFormatter.ISO_LOCAL_DATE).appendLiteral("T").appendValue(ChronoField.HOUR_OF_DAY, 2).appendLiteral(":").appendValue(ChronoField.MINUTE_OF_HOUR, 2).appendLiteral(":").appendValue(ChronoField.SECOND_OF_MINUTE, 2).appendFraction(ChronoField.NANO_OF_SECOND, minDigits, maxDigits, true).appendOffsetId().toFormatter()._toPrinterParser(false).parse(newContext, text, position);
			if (pos < 0) return pos;
			...
			var instantSecs;
			try {
				instantSecs = LocalDateTime.of(year, month, day, hour, min, sec, 0).plusDays(days).toEpochSecond(ZoneOffset.ofTotalSeconds(offset));
	...
	YearMonth = function(_Temporal) {
		...
		_proto._plusUnit = function _plusUnit(amountToAdd, unit) {
			...
			if (unit instanceof ChronoUnit) {
				switch (unit) {
					...
					case ChronoUnit.MILLENNIA: return this.plusYears(MathUtil.safeMultiply(amountToAdd, 1e3));
		...
		_proto.plusMonths = function plusMonths(monthsToAdd) {
			if (monthsToAdd === 0) return this;
			var calcMonths = this._year * 12 + (this._month - 1) + monthsToAdd;
		...
		_proto.until = function until(endExclusive, unit) {
			...
			if (unit instanceof ChronoUnit) {
				...
				switch (unit) {
					...
					case ChronoUnit.MILLENNIA: return MathUtil.intDiv(monthsUntil, 12e3);
	...
	Year = function(_Temporal) {
		...
		Year.now = function now(zoneIdOrClock) {
			if (zoneIdOrClock === void 0) zoneIdOrClock = void 0;
			if (zoneIdOrClock === void 0) return Year.now0();
		...
		_proto._withField = function _withField(field, newValue) {
			...
			if (field instanceof ChronoField) {
				...
				switch (field) {
					case ChronoField.YEAR_OF_ERA: return Year.of(this._year < 1 ? 1 - newValue : newValue);
					case ChronoField.YEAR: return Year.of(newValue);
					case ChronoField.ERA: return this.getLong(ChronoField.ERA) === newValue ? this : Year.of(1 - this._year);
				}
				throw new UnsupportedTemporalTypeException("Unsupported field: " + field);
			}
			return field.adjustInto(this, newValue);
		...
		_proto.until = function until(endExclusive, unit) {
			...
			if (unit instanceof ChronoUnit) {
				...
				switch (unit) {
					case ChronoUnit.YEARS: return yearsUntil;
					case ChronoUnit.DECADES: return MathUtil.intDiv(yearsUntil, 10);
					case ChronoUnit.CENTURIES: return MathUtil.intDiv(yearsUntil, 100);
					case ChronoUnit.MILLENNIA: return MathUtil.intDiv(yearsUntil, 1e3);
					case ChronoUnit.ERAS: return end.getLong(ChronoField.ERA) - this.getLong(ChronoField.ERA);
				}
				throw new UnsupportedTemporalTypeException("Unsupported unit: " + unit);
			}
	...
	Impl = function(_TemporalAdjuster) {
		function Impl(ordinal) {
			var _this = _TemporalAdjuster.call(this) || this;
			_this._ordinal = ordinal;
			return _this;
		}
	...
	DayOfWeekInMonth = function(_TemporalAdjuster2) {
		function DayOfWeekInMonth(ordinal, dow) {
			var _this2 = _TemporalAdjuster2.call(this) || this;
	...
	RelativeDayOfWeek = function(_TemporalAdjuster3) {
		function RelativeDayOfWeek(relative, dayOfWeek) {
			var _this3 = _TemporalAdjuster3.call(this) || this;
			requireNonNull(dayOfWeek, "dayOfWeek");
		...
		_proto.resolveDate = function resolveDate(fieldValues, resolverStyle) {
			...
			if (yoeLong != null) {
				...
				if (era == null) {
					...
					if (resolverStyle === ResolverStyle.STRICT) if (year != null) this._updateResolveMap(fieldValues, ChronoField.YEAR, year > 0 ? yoeLong : MathUtil.safeSubtract(1, yoeLong));
	...
	OffsetTime = function(_Temporal) {
		...
		OffsetTime.ofInstant = function ofInstant(instant, zone) {
			...
			return new OffsetTime(LocalTime.ofSecondOfDay(secsOfDay, instant.nano()), offset);
		};
		...
		_proto.until = function until(endExclusive, unit) {
			...
			if (unit instanceof ChronoUnit) {
				...
				switch (unit) {
					...
					case ChronoUnit.MICROS: return MathUtil.intDiv(nanosUntil, 1e3);
					case ChronoUnit.MILLIS: return MathUtil.intDiv(nanosUntil, 1e6);
	...
	ChronoZonedDateTime = function(_Temporal) {
		...
		_proto.toEpochSecond = function toEpochSecond() {
			var secs = this.toLocalDate().toEpochDay() * 86400 + this.toLocalTime().toSecondOfDay();
	...
	ZonedDateTime = function(_ChronoZonedDateTime) {
		...
		_proto._plusUnit = function _plusUnit(amountToAdd, unit) {
			if (unit instanceof ChronoUnit) if (unit.isDateBased()) return this._resolveLocal(this._dateTime.plus(amountToAdd, unit));
		...
		_proto._withField = function _withField(field, newValue) {
			...
			if (field instanceof ChronoField) {
				...
				switch (f) {
					...
					case ChronoField.OFFSET_SECONDS: return this._withDateTimeOffset(this._dateTime, ZoneOffset.ofTotalSeconds(f.checkValidIntValue(newValue)));
				}
	...
	DAYS_0000_TO_1970 = DAYS_PER_CYCLE * 5 - 10957;
	LocalDate = function(_ChronoLocalDate) {
		function LocalDate(year, month, dayOfMonth) {
			var _this = _ChronoLocalDate.call(this) || this;
			requireNonNull(year, "year");
			requireNonNull(month, "month");
			requireNonNull(dayOfMonth, "dayOfMonth");
		...
		LocalDate.ofYearDay = function ofYearDay(year, dayOfYear) {
			...
			if (dayOfYear > moy.firstDayOfYear(leap) + moy.length(leap) - 1) moy = moy.plus(1);
			var dom = dayOfYear - moy.firstDayOfYear(leap) + 1;
			return new LocalDate(year, moy.value(), dom);
		};
		LocalDate.ofEpochDay = function ofEpochDay(epochDay) {
			if (epochDay === void 0) epochDay = 0;
			var adjust, adjustCycles, doyEst, yearEst, zeroDay = epochDay + DAYS_0000_TO_1970;
			zeroDay -= 60;
			adjust = 0;
			if (zeroDay < 0) {
		...
		LocalDate._validate = function _validate(year, month, dayOfMonth) {
			...
			if (dayOfMonth > 28) {
				...
				if (dayOfMonth > dom) if (dayOfMonth === 29) assert(false, "Invalid date 'February 29' as '" + year + "' is not a leap year", DateTimeException);
			...
			if (field instanceof ChronoField) {
				if (field.isDateBased()) {
					switch (field) {
						...
						case ChronoField.YEAR_OF_ERA: return this._year <= 0 ? ValueRange.of(1, Year.MAX_VALUE + 1) : ValueRange.of(1, Year.MAX_VALUE);
					}
					return field.range();
				}
				throw new UnsupportedTemporalTypeException("Unsupported field: " + field);
		...
		_proto.until2 = function until2(endExclusive, unit) {
			...
			if (unit instanceof ChronoUnit) {
				switch (unit) {
					...
					case ChronoUnit.MILLENNIA: return MathUtil.intDiv(this._monthsUntil(end), 12e3);
					case ChronoUnit.ERAS: return end.getLong(ChronoField.ERA) - this.getLong(ChronoField.ERA);
				}
				throw new UnsupportedTemporalTypeException("Unsupported unit: " + unit);
			}
		...
		_proto.toString = function toString() {
			...
			var monthValue = this._month;
			var dayValue = this._day;
			if (Math.abs(yearValue) < 1e3) if (yearValue < 0) yearString = "-" + ("" + (yearValue - 1e4)).slice(-4);
			else yearString = ("" + (yearValue + 1e4)).slice(-4);
	...
	LocalDateTime = function(_ChronoLocalDateTime) {
		function LocalDateTime(date, time) {
			var _this = _ChronoLocalDateTime.call(this) || this;
		...
		_proto._withField = function _withField(field, newValue) {
			requireNonNull(field, "field");
			if (field instanceof ChronoField) if (field.isTimeBased()) return this._withDateTime(this._date, this._time.with(field, newValue));
		...
		_proto._plusUnit = function _plusUnit(amountToAdd, unit) {
			...
			if (unit instanceof ChronoUnit) {
				switch (unit) {
					case ChronoUnit.NANOS: return this.plusNanos(amountToAdd);
					case ChronoUnit.MICROS: return this.plusDays(MathUtil.intDiv(amountToAdd, LocalTime.MICROS_PER_DAY)).plusNanos(MathUtil.intMod(amountToAdd, LocalTime.MICROS_PER_DAY) * 1e3);
					case ChronoUnit.MILLIS: return this.plusDays(MathUtil.intDiv(amountToAdd, LocalTime.MILLIS_PER_DAY)).plusNanos(MathUtil.intMod(amountToAdd, LocalTime.MILLIS_PER_DAY) * 1e6);
					case ChronoUnit.SECONDS: return this.plusSeconds(amountToAdd);
					case ChronoUnit.MINUTES: return this.plusMinutes(amountToAdd);
					case ChronoUnit.HOURS: return this.plusHours(amountToAdd);
					case ChronoUnit.HALF_DAYS: return this.plusDays(MathUtil.intDiv(amountToAdd, 256)).plusHours(MathUtil.intMod(amountToAdd, 256) * 12);
				}
	...
	LocalTime = function(_Temporal) {
		...
		_proto._get0 = function _get0(field) {
			switch (field) {
				...
				case ChronoField.MICRO_OF_SECOND: return MathUtil.intDiv(this._nano, 1e3);
				case ChronoField.MICRO_OF_DAY: return MathUtil.intDiv(this.toNanoOfDay(), 1e3);
				case ChronoField.MILLI_OF_SECOND: return MathUtil.intDiv(this._nano, 1e6);
				case ChronoField.MILLI_OF_DAY: return MathUtil.intDiv(this.toNanoOfDay(), 1e6);
		...
		_proto._withField = function _withField(field, newValue) {
			...
			if (field instanceof ChronoField) {
				...
				switch (field) {
					...
					case ChronoField.MICRO_OF_SECOND: return this.withNano(newValue * 1e3);
					case ChronoField.MICRO_OF_DAY: return LocalTime.ofNanoOfDay(newValue * 1e3);
					case ChronoField.MILLI_OF_SECOND: return this.withNano(newValue * 1e6);
					case ChronoField.MILLI_OF_DAY: return LocalTime.ofNanoOfDay(newValue * 1e6);
		...
		_proto._plusUnit = function _plusUnit(amountToAdd, unit) {
			...
			if (unit instanceof ChronoUnit) {
				switch (unit) {
					case ChronoUnit.NANOS: return this.plusNanos(amountToAdd);
					case ChronoUnit.MICROS: return this.plusNanos(MathUtil.intMod(amountToAdd, LocalTime.MICROS_PER_DAY) * 1e3);
					case ChronoUnit.MILLIS: return this.plusNanos(MathUtil.intMod(amountToAdd, LocalTime.MILLIS_PER_DAY) * 1e6);
					case ChronoUnit.SECONDS: return this.plusSeconds(amountToAdd);
					case ChronoUnit.MINUTES: return this.plusMinutes(amountToAdd);
					case ChronoUnit.HOURS: return this.plusHours(amountToAdd);
		...
		_proto.plusHours = function plusHours(hoursToAdd) {
			...
			return new LocalTime(MathUtil.intMod(MathUtil.intMod(hoursToAdd, LocalTime.HOURS_PER_DAY) + this._hour + LocalTime.HOURS_PER_DAY, LocalTime.HOURS_PER_DAY), this._minute, this._second, this._nano);
		};
		_proto.plusMinutes = function plusMinutes(minutesToAdd) {
			...
			return new LocalTime(MathUtil.intDiv(newMofd, LocalTime.MINUTES_PER_HOUR), MathUtil.intMod(newMofd, LocalTime.MINUTES_PER_HOUR), this._second, this._nano);
		};
		_proto.plusSeconds = function plusSeconds(secondsToAdd) {
			...
			return new LocalTime(MathUtil.intDiv(newSofd, LocalTime.SECONDS_PER_HOUR), MathUtil.intMod(MathUtil.intDiv(newSofd, LocalTime.SECONDS_PER_MINUTE), LocalTime.MINUTES_PER_HOUR), MathUtil.intMod(newSofd, LocalTime.SECONDS_PER_MINUTE), this._nano);
		};
		_proto.plusNanos = function plusNanos(nanosToAdd) {
		...
		_proto.toString = function toString() {
			...
			if (secondValue > 0 || nanoValue > 0) {
				...
				if (nanoValue > 0) {
					buf += ".";
					if (MathUtil.intMod(nanoValue, 1e6) === 0) buf += ("" + (MathUtil.intDiv(nanoValue, 1e6) + 1e3)).substring(1);
					else if (MathUtil.intMod(nanoValue, 1e3) === 0) buf += ("" + (MathUtil.intDiv(nanoValue, 1e3) + 1e6)).substring(1);
					else buf += ("" + (nanoValue + 1e9)).substring(1);
				}
			}
			return buf;
		};
	...
	Instant = function(_Temporal) {
		...
		Instant.ofEpochMilli = function ofEpochMilli(epochMilli) {
			...
			return Instant._create(secs, mos * 1e6);
		};
		Instant.ofEpochMicro = function ofEpochMicro(epochMicro) {
			...
			return Instant._create(secs, mos * 1e3);
		};
		...
		_proto.getLong = function getLong(field) {
			if (field instanceof ChronoField) {
				switch (field) {
					...
					case ChronoField.MICRO_OF_SECOND: return MathUtil.intDiv(this._nanos, 1e3);
		...
		_proto.toEpochMilli = function toEpochMilli() {
			return MathUtil.safeMultiply(this._seconds, 1e3) + MathUtil.intDiv(this._nanos, NANOS_PER_MILLI);
		};
	...
	FixedClock = function(_Clock2) {
		function FixedClock(fixedInstant, zoneId) {
			var _this2 = _Clock2.call(this) || this;
	...
	OffsetClock = function(_Clock3) {
		function OffsetClock(baseClock, offset) {
			var _this3 = _Clock3.call(this) || this;
	...
	ZoneOffsetTransition = function() {
		...
		_proto.validOffsets = function validOffsets() {
			...
			else return [this._offsetBefore, this._offsetAfter];
		};
	...
	SystemDefaultZoneRules = function(_ZoneRules) {
		...
		_proto.offsetOfLocalDateTime = function offsetOfLocalDateTime(localDateTime) {
			...
			var epochMilliSystemZone = epochMilli + new Date(epochMilli).getTimezoneOffset() * 6e4;
	...
	ToNativeJsConverter = function() {
		function ToNativeJsConverter(temporal, zone) {
			...
			} else if (temporal instanceof ZonedDateTime) if (zone == null) zonedDateTime = temporal;
...
//#endregion
export { js_joda_esm_exports as n, init_js_joda_esm as t };
