//#region node_modules/.nitro/vite/services/ssr/assets/syspro-programs-CuezAD_5.js
var CATALOG = {
	IMP010: {
		name: "Inventory import",
		description: "Imports stock / inventory transactions into SYSPRO. Failures often block warehouse and finance posting.",
		area: "Inventory"
	},
	IMP041: {
		name: "Import processing",
		description: "Related import / interface step. Check with IMP010 if errors cluster on the same operators.",
		area: "Inventory"
	},
	IMPFRM: {
		name: "Import forms",
...
function extractProgramCode(text) {
	...
	if (!m) return null;
	const c = m[1];
	if (/^[A-Z]{3}/.test(c) || CATALOG[c] || PREFIX_HINTS.some((p) => c.startsWith(p.prefix))) return c;
	return null;
}
function fromPrefix(code) {
	for (const p of PREFIX_HINTS) if (code.startsWith(p.prefix)) return {
		name: `${p.name} (${code})`,
...
function getSysproProgram(code) {
	...
	return {
		code: c,
		name: "SYSPRO program",
		description: "Program code from job logging or AMS priority. Add a friendly name in the catalogue if this appears often.",
		area: "Other"
	};
}
/** "IMP010 — Inventory import" */
function formatProgramLabel(code) {
...
//#endregion
export { formatProgramLabel as n, getSysproProgram as r, extractProgramCode as t };
