...
export function CardHead({ className, children, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "rpma-panel-heading flex items-center border-b border-border px-4 py-3 font-extrabold tracking-tight text-fg",
        className,
      )}
      {...props}
    >
...
export function CardTitle({ className, ...props }: React.ComponentProps<"h3">) {
  return (
    <h3
      className={cn(
        "text-sm font-extrabold leading-none tracking-tight text-fg sm:text-[0.95rem]",
        className,
      )}
      {...props}
    />
