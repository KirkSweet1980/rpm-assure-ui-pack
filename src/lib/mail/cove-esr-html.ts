...
function machineTable(esr: CoveEsr): string {
  if (!esr.machines.length) {
    return `<p class="muted">No machine names on the latest Cloud Backup collect.</p>`;
  }
...
export function coveEsrSections(esr: CoveEsr): string {
  if (!esr.covered) {
    return `<p class="note"><strong class="warn">No cover</strong> — no RPM Cloud Backup backup data for this customer.</p>`;
  }
  const sgRows: string[] = [];
