...
function linksForPage(ctx: AssistantContext): AssistantLink[] {
  ...
  return [
    ...
    { label: "RPM Service Desk", href: `${base}/tickets` },
...
function pageHelp(path: string): string | null {
  ...
  if (/\/customers\/[^/]+\/epp/.test(path)) {
    return "RPM EndPoint Protection covers endpoints, policies, security incidents and quarantine. No Cover means no endpoints are mapped.";
  }
...
export function answerLocally(question: string, ctx: AssistantContext): AssistantReply {
  ...
  if (q.includes("cover") && (q.includes("what") || q.includes("mean") || q.includes("no cover"))) {
    return {
      source: "local",
      text:
        "Cover means the tenant is in scope for that RPM Service and we collect data. No Cover (pewter) means it is not scored, not billed as that service, and does not move estate RAG. Green = Cover. Indigo = the selected item. Map RMM / RPM Cloud Backup / RPM EndPoint Protection / CSP in Configuration → Integrations, then run collect.",
  ...
  const go: [RegExp, string, (b: string) => string][] = [
    ...
    [/rmm sla|uptime sla/, "RMM Service SLA", (b) => `${b}/rmm/sla`],
    [/backup sla|cove sla|rpo/, "Backup Service SLA", (b) => `${b}/cove/sla`],
    [/epp sla|endpoint sla/, "Endpoint Service SLA", (b) => `${b}/epp/sla`],
    [/sla/, "SLA", (b) => `${b}/ams/sla`],
