...
export function finalizeEstateHealth(input: {
  ...
  epp?: { rag: HealthRag; summary: string } | null;
}): { rag: HealthRag; summary: string } {
...
export function eppHealthFor(input: {
  deviceCount: number;
  unmanagedCount?: number;
  infectedCount?: number;
}): { rag: HealthRag; summary: string } {
  const n = input.deviceCount || 0;
  const unmanaged = input.unmanagedCount || 0;
  const infected = input.infectedCount || 0;
  if (n <= 0) {
    return { rag: "Amber", summary: "RPM EPP covered — no endpoint snapshot yet." };
  }
  if (infected > 0) {
    return {
      rag: "Red",
      summary: `RPM EPP: ${infected} infected of ${n} endpoint(s).`,
    };
  }
  if (unmanaged > 0) {
    return {
      rag: "Amber",
      summary: `RPM EPP: ${unmanaged} unmanaged of ${n} endpoint(s).`,
    };
  }
  return {
    rag: "Green",
    summary: `RPM EPP healthy — ${n} endpoint(s) managed.`,
  };
}
