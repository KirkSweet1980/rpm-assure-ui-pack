    ...
    try {
      ...
      if (pool) {
        ...
        if (row && row.IsActive) {
          ...
          let tenantAccess: TenantGrant[] = [];
          if (role !== "PlatformAdmin") {
            try {
              const cust = await pool
                .request()
                .input("id", sql.UniqueIdentifier, row.AppUserId)
                .query<{ CustomerCode: string; Role: string; Pillars: string | null }>(`
                  SELECT CustomerCode, Role, Pillars
                  FROM dbo.App_UserCustomer
                  WHERE AppUserId = @id
                `);
              tenantAccess = (cust.recordset ?? []).map((c) => ({
                code: String(c.CustomerCode),
                role: isTenantRole(c.Role) ? c.Role : "Operator",
                services: parseServices(c.Pillars),
              }));
            } catch {
              const cust = await pool
                .request()
                .input("id", sql.UniqueIdentifier, row.AppUserId)
                .query<{ CustomerCode: string; Role: string }>(`
                  SELECT CustomerCode, Role
                  FROM dbo.App_UserCustomer
                  WHERE AppUserId = @id
                `);
              tenantAccess = (cust.recordset ?? []).map((c) => ({
            ...
            allowed = tenantAccess.map((g) => g.code);
          }
