...
function Raw($body) {
  ...
  $r = Invoke-WebRequest -Uri $ApiUrl -Method POST -Body ([Text.Encoding]::UTF8.GetBytes($j)) `
    -ContentType 'application/json; charset=utf-8' -UseBasicParsing -TimeoutSec 120
  return $r.Content
}

$login = Raw @{ jsonrpc='2.0'; method='Login'; params=@{ username=$Username; password=$Password }; id='1' }
$visa = ([regex]::Match($login, '"visa"\s*:\s*"([^"]+)"')).Groups[1].Value
if (-not $visa) { throw 'login no visa' }
$pid0 = 0
[void][int]::TryParse(([regex]::Match($login, '"PartnerId"\s*:\s*(\d+)').Groups[1].Value), [ref]$pid0)
...
$rows = @($obj.result.result)
if (-not $rows.Count) { $rows = @($obj.result) }
...
if (-not $hit) { $hit = $rows | Where-Object { (GS $_ 'I80') -in @('1','2') } | Select-Object -First 3 }
foreach ($r in $hit) {
  Write-Host "----"
