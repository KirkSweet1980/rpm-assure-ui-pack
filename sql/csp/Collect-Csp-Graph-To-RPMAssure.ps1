...
function Invoke-SqlFile([string]$path) {
  $sqlcmd = Find-Sqlcmd
  if ($script:UseWinAuth) {
    & $sqlcmd -S $SqlServer -d $SqlDatabase -E -C -b -i $path
  } else {
    & $sqlcmd -S $SqlServer -d $SqlDatabase -U $SqlUser -P $SqlPassword -C -b -i $path
  }
  if ($LASTEXITCODE -ne 0) { throw "sqlcmd failed exit=$LASTEXITCODE on $path" }
}
...
function Invoke-SqlText([string]$sql, [string]$label) {
  ...
  if ($script:UseWinAuth) {
    & $sqlcmd -S $SqlServer -d $SqlDatabase -E -C -b -i $f
  } else {
...
if (-not [string]::IsNullOrWhiteSpace($ConfigPath) -and (Test-Path -LiteralPath $ConfigPath)) {
  ...
  $defaultCfg = Join-Path $here "Csp.Config.ps1"
  if (Test-Path -LiteralPath $defaultCfg) { $cfg = $defaultCfg }
}
if ($cfg) {
  . $cfg
...
if (Get-Variable -Name WindowsAuth -EA SilentlyContinue) {
  if ($WindowsAuth -eq $true) { $script:UseWinAuth = $true }
}
if ([string]::IsNullOrWhiteSpace($SqlPassword)) { $script:UseWinAuth = $true }
