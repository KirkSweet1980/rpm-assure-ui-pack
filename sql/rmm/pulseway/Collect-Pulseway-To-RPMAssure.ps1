...
function Enrich-DevicePatch($d, [string]$did) {
  ...
  if ($fromU) {
    if (Set-DevicePatchNote $d $fromU 'Updates-object') { $got = $true }
  }

  $c0 = Get-PatchCountsFromObject $d
  if ($c0) { if (Set-DevicePatchNote $d $c0 'device-object') { $got = $true } }
  ...
  $hasItems = $d.PSObject.Properties.Name -contains 'PatchItems' -and @($d.PatchItems).Count -gt 0 -and ($d.PatchItems | Where-Object { $_.Kb -or ($_.Title -match 'KB\d{5,}') })
  $doList = $looksServer -and ($script:PatchListN -lt 80) -and -not $hasItems
  if (-not $doList) {
    if ($hasItems) { return $got }
    if (-not $script:PatchProbeN) { $script:PatchProbeN = 0 }
    if ($script:PatchProbeN -ge 4) { return $got }
    $script:PatchProbeN++
  } else {
    $script:PatchListN++
  }
  ...
  $paths = @(
    ...
    ("devices/{0}/patches" -f $did),
    ("devices/{0}/osupdates" -f $did),
  ...
  foreach ($path in $paths) {
    ...
    if ($counts) {
      if (Set-DevicePatchNote $d $counts $path) { $got = $true }
      if ($counts.Items -and @($counts.Items).Count -gt 0) { return $true }
    }
  }
  return $got
}
