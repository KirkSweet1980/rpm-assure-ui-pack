/*
  CENTRAL — force measured SLA numbers for AHIC current month
  Fixes empty Availability panel ("Awaiting feed" / n/a).

  sqlcmd -S "102.222.21.220,14333" -d "RPMAssure_App" -E -C -i 244_Upsert_AHIC_SlaMeasured.sql
*/
USE RPMAssure_App;
GO
SET NOCOUNT ON;
...
BEGIN
  RAISERROR(N'AHIC not active in Dim_Customer', 16, 1);
  RETURN;
END
...
BEGIN
  ...
  WHERE CustomerCode = N'AHIC' AND PeriodFrom = @from AND PeriodTo = @to;
  PRINT N'Updated AHIC Fact_DashboardSnapshot measured SLA for current month.';
END
ELSE
BEGIN
  ...
  VALUES
  (
    N'AHIC', @from, @to, @label, SYSUTCDATETIME(),
    N'Amber', N'Measured sample for AMS UI (replace with monitoring feed).',
    99.820, 99.500,
    96.000, 94.000, 95.000
  );
  PRINT N'Inserted AHIC Fact_DashboardSnapshot measured SLA for current month.';
END
