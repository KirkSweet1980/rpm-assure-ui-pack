/*
  CENTRAL — sample Fact AMS rows for UVSS (safe to re-run: deletes previous seed tags)
  sqlcmd -S "102.222.21.220,14333" -d "RPMAssure_App" -E -C -i 240_Seed_UVSS_FactAms.sql
*/
USE RPMAssure_App;
GO
SET NOCOUNT ON;
...
BEGIN
  RAISERROR(N'UVSS missing from Dim_Customer', 16, 1);
  RETURN;
END
...
VALUES
  (N'UVSS', N'IMP010 SQL transaction conflicts during import', N'Medium', N'InProgress', N'Medium',
   DATEADD(DAY, -2, SYSUTCDATETIME()), 0, N'SEED-INC-001',
   N'Repeat job errors on stock/import programmes; operators re-run batch.', N'INV'),
  (N'UVSS', N'Datarapt CB out-of-balance elevated pre month-end', N'High', N'New', N'High',
...
VALUES
  (N'UVSS', N'[SEED] Recurring IMP010 transaction nesting', N'Medium', N'Investigating',
...
VALUES
  (N'UVSS', N'[SEED] Month-end blocked by CB/DN DTR variances', N'Application', N'Amber', N'Open',
   N'AMS Lead', CAST(DATEADD(DAY, 10, SYSUTCDATETIME()) AS date),
   N'Cashbook and Dispatch Notes show high out-of-balance counts.'),
  (N'UVSS', N'[SEED] License expiry tracking', N'Commercial', N'Green', N'Mitigating',
...
VALUES
  (N'UVSS', N'[SEED] ASS DTR collect column map incomplete', N'Internal', N'Low', N'InProgress',
...
VALUES
  (N'UVSS', N'[SEED] Clear CB and DN out-of-balance lines', N'Work with finance on Datarapt refresh and open journals.', 1, N'Active', N'Aug 2026'),
  (N'UVSS', N'[SEED] Reduce IMP010 job error recurrence', N'Root-cause with vendor/internal on transaction nesting.', 2, N'Active', N'Aug 2026'),
  (N'UVSS', N'[SEED] Confirm SYSPRO health check 14 OOB items', N'Align AdmSysHealthLog message with DTR action list.', 3, N'Active', N'Aug 2026');

SELECT 'Incidents' t, COUNT(*) c FROM dbo.Fact_Incident WHERE CustomerCode=N'UVSS'
UNION ALL SELECT 'Problems', COUNT(*) FROM dbo.Fact_Problem WHERE CustomerCode=N'UVSS'
UNION ALL SELECT 'Risks', COUNT(*) FROM dbo.Fact_Risk WHERE CustomerCode=N'UVSS'
UNION ALL SELECT 'Issues', COUNT(*) FROM dbo.Fact_Issue WHERE CustomerCode=N'UVSS'
UNION ALL SELECT 'Priorities', COUNT(*) FROM dbo.Fact_Priority WHERE CustomerCode=N'UVSS';
GO
