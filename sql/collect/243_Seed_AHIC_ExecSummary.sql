/*
  CENTRAL - Executive summary + optional measured SLA snapshot for AHIC
  sqlcmd -S "102.222.21.220,14333" -d "RPMAssure_App" -E -C -i 243_Seed_AHIC_ExecSummary.sql
*/
USE RPMAssure_App;
GO
SET NOCOUNT ON;
...
BEGIN RAISERROR(N'AHIC missing',16,1); RETURN; END
...
WHERE s.CustomerCode = N'AHIC' AND s.PeriodFrom = @from AND s.PeriodTo = @to
...
BEGIN
  IF EXISTS (SELECT 1 FROM dbo.Fact_DashboardSnapshot WHERE CustomerCode=N'AHIC' AND PeriodFrom=@from AND PeriodTo=@to)
  ...
  ELSE
    ...
    VALUES
    (N'AHIC', @from, @to, @label, SYSUTCDATETIME(), N'Amber',
     N'[SEED] Sample measured availability/SLA - replace with monitoring feed.',
     99.82, 99.500, 96.0, 94.0, 95.0);
END

SELECT 'ExecSummary' t, COUNT(*) c FROM dbo.Fact_ExecSummary WHERE CustomerCode=N'AHIC'
UNION ALL SELECT 'Narratives', COUNT(*) FROM dbo.Fact_ExecNarrative n
  INNER JOIN dbo.Fact_ExecSummary s ON s.ExecSummaryId=n.ExecSummaryId WHERE s.CustomerCode=N'AHIC';

SELECT PeriodLabel, HealthRag, Status FROM dbo.Fact_ExecSummary WHERE CustomerCode=N'AHIC' ORDER BY PeriodTo DESC;
SELECT AvailabilityPct, SlaCompliancePct FROM dbo.Fact_DashboardSnapshot WHERE CustomerCode=N'AHIC' AND PeriodFrom=@from;
GO
