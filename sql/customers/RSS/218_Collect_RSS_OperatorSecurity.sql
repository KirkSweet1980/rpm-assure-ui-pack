/*
  RSS security collect (site-tuned for discovered columns)
  ...
  sqlcmd -S "." -U rpmassure -P "..." -C -b -i thisfile.sql
*/
SET NOCOUNT ON;
SET XACT_ABORT OFF;
...
PRINT CONCAT(N'=== RSS operator security ', CONVERT(char(10), @SnapshotDate, 23), N' ===');

IF NOT EXISTS (
    SELECT 1 FROM [RPM_CENTRAL].[RPMAssure_App].dbo.Dim_Customer
    WHERE CustomerCode = @CustomerCode AND Active = 1)
BEGIN
    RAISERROR(N'RSS not active.', 16, 1);
    RETURN;
END;
...
PRINT N'=== Done RSS operator security ===';
GO
