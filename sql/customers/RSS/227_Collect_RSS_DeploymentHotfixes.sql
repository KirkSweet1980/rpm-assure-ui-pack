/*
  RSS — TRUE installed hotfixes from SYSPRODeployment
  Maps:
...
PRINT CONCAT(N'=== RSS Deployment Hotfixes ', CONVERT(char(10), @SnapshotDate, 23), N' ===');

IF NOT EXISTS (
  SELECT 1 FROM [RPM_CENTRAL].[RPMAssure_App].dbo.Dim_Customer
  WHERE CustomerCode = @CustomerCode AND Active = 1)
BEGIN
  PRINT N'RSS not active on central.';
  RETURN;
END
...
PRINT N'=== Done RSS Deployment Hotfixes ===';
GO
