/*
  RSS collect — operators + LastLoginDate (deduped by OperatorCode)
  Run ON RSS-PROD as rpmassure.
*/
SET NOCOUNT ON;
SET XACT_ABORT OFF;
...
PRINT CONCAT(N'=== RSS collect+login start ', CONVERT(char(10), @SnapshotDate, 23), N' ===');

IF NOT EXISTS (
    SELECT 1 FROM [RPM_CENTRAL].[RPMAssure_App].dbo.Dim_Customer
    WHERE CustomerCode = @CustomerCode AND Active = 1)
BEGIN
    RAISERROR(N'RSS not found/active on central Dim_Customer.', 16, 1);
    RETURN;
END;
...
VALUES (N'SysproCollectStart', @CustomerCode,
    CONCAT(N'RSS operators+login snap=', CONVERT(char(10), @SnapshotDate, 23)), 0);
...
VALUES (N'SysproCollectEnd', @CustomerCode,
    CONCAT(N'RSS complete snap=', CONVERT(char(10), @SnapshotDate, 23),
           N' withLogin=', @withLogin), 0);

PRINT N'=== Done RSS (operators+login) ===';
GO
