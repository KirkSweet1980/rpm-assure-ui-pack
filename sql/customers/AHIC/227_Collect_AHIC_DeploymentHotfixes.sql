/*
  AHIC — TRUE installed hotfixes from SYSPRODeployment
  Maps:
    HotfixCode  <- KB from HotfixFileName (e.g. KB8111156) or file name
    HotfixName  <- HotfixSynopsis (short)
    Description  <- HotfixSynopsis
  sqlcmd -S "." -U "Rpm_collect" -P "..." -C -b -i 227_Collect_AHIC_DeploymentHotfixes.sql
*/
SET NOCOUNT ON;
SET XACT_ABORT OFF;
SET ANSI_NULLS ON;
...
PRINT CONCAT(N'=== AHIC Deployment Hotfixes ', CONVERT(char(10), @SnapshotDate, 23), N' ===');

IF NOT EXISTS (
  SELECT 1 FROM [RPM_CENTRAL].[RPMAssure_App].dbo.Dim_Customer
  WHERE CustomerCode = @CustomerCode AND Active = 1)
BEGIN
  PRINT N'AHIC not active on central.';
  RETURN;
END
...
PRINT N'=== Done AHIC Deployment Hotfixes ===';
GO
