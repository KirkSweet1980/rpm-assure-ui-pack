/*
  UVSS collect — operators + LastLoginDate (deduped by OperatorCode)
  Run ON UVSS-SYSPRO as Rpm_collect.
*/
SET NOCOUNT ON;
SET XACT_ABORT OFF;
...
PRINT CONCAT(N'=== UVSS collect+login start ', CONVERT(char(10), @SnapshotDate, 23), N' ===');

IF NOT EXISTS (
    SELECT 1 FROM [RPM_CENTRAL].[RPMAssure_App].dbo.Dim_Customer
    WHERE CustomerCode = @CustomerCode AND Active = 1)
BEGIN
    RAISERROR(N'UVSS not found/active on central Dim_Customer.', 16, 1);
    RETURN;
END;
...
PRINT N'=== Done UVSS (operators+login) ===';
GO
