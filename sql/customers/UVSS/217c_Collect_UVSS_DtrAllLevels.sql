/*
  UVSS — FinSight DTR Levels 1+2+3 (all company DBs) → central Syspro_Dtr*
  Run ON the customer SYSPRO SQL host (linked server RPM_CENTRAL).

  powershell -NoProfile -ExecutionPolicy Bypass -File C:\RPM-Assure\Sql\collect\Run-Dtr-AllLevels.ps1 -CustomerCode UVSS
*/
SET NOCOUNT ON;
SET XACT_ABORT OFF;
...
PRINT CONCAT(N'=== UVSS DTR L1+L2+L3 ', CONVERT(char(10), @SnapshotDate, 23), N' ===');

IF NOT EXISTS (
    SELECT 1 FROM [RPM_CENTRAL].[RPMAssure_App].dbo.Dim_Customer
    WHERE CustomerCode = @CustomerCode AND Active = 1)
BEGIN
    RAISERROR(N'UVSS not active on central Dim_Customer.', 16, 1);
    RETURN;
END;
...
PRINT CONCAT(N'=== UVSS DTR L1-3 total rows=', @total, N' ===');
