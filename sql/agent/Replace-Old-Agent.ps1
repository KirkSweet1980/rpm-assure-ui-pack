...
$sqlc = Join-Path $Pack "Sql\agent\installer\Sql-Connect.ps1"
if (Test-Path $sqlc) { Copy-Item $sqlc (Join-Path $stage "Sql-Connect.ps1") -Force }
...
$centralUser = "Rpm_collect"
$centralPass = $env:RPM_ASSURE_SQL_PASSWORD
if ([string]::IsNullOrWhiteSpace($centralPass) -and (Test-Path 'C:\RPM-Assure\secrets\sql-collect.json')) {
  try { $centralPass = [string]((Get-Content 'C:\RPM-Assure\secrets\sql-collect.json' -Raw | ConvertFrom-Json).password) } catch {}
}
if ([string]::IsNullOrWhiteSpace($centralPass)) { $centralPass = '' }
