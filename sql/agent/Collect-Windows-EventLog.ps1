...
W "START eventlog host=$HostName customer=$CustomerCode lookback=${LookbackHours}h https=$PreferHttps"
...
foreach ($e in $found) {
  ...
  try { $msg = [string]$e.Message } catch { $msg = "" }
  $evObjs += @{
    timeCreatedUtc = $e.TimeCreated.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss")
...
if (Get-Command Send-RpmaHttpsEvents -ErrorAction SilentlyContinue) {
  try {
    $er = Send-RpmaHttpsEvents -HostName $HostName -CustomerCode $CustomerCode -Events $evObjs
    if ($er.StatusCode -ge 200 -and $er.StatusCode -lt 300) {
      W ("DONE https events=$($found.Count) snap=$snap")
      exit 0
    }
    W ("WARN https events " + $er.Text)
  } catch { W ("WARN https events " + $_.Exception.Message) }
}
if ($PreferHttps) {
  W "FAIL https events required (no SQL fallback)"
  exit 1
}
$sql = @"
SET NOCOUNT ON;
