...
# Passwords MUST come from -ConfigFile (never CLI) so  is not splatted.
# ASCII only.
param(
  [string]$ConfigFile = "",
  [string]$LocalServer = "",
...
if (-not $CollectPassword) { throw 'CollectPassword required (ConfigFile / secrets — no default in git)' }
