...
if (-not $CustomerCode) { throw "CustomerCode missing in Agent.Config.ps1" }
...
# --- Job catalog (local config overrides) ---
# Jobs: name, interval minutes, script path (absolute or under AgentRoot)
$jobs = @()
if ($AgentJobs -and $AgentJobs.Count -gt 0) {
  $jobs = $AgentJobs
} else {
  $cfgPath = Join-Path $SqlRoot "customers\$CustomerCode\Customer.Config.ps1"
  if (-not (Test-Path $cfgPath)) {
    $cfgPath = Join-Path $SqlRoot "base\syspro-direct\Customer.Config.ps1"
  }
  ...
  if ($RoleTags -match 'syspro') {
    if (Test-Path $sysproRunner) {
      $jobs += @{
        Name = "syspro-core"
        IntervalMin = 15
        Script = $sysproRunner
        Args = @("-ConfigPath", $cfgPath, "-JobsErrorsOnly")
      }
    }
...
function Report-JobRun {
  param($Name, $Started, $Finished, $ExitCode, $Message, $LogTail)
...
VALUES ($(Sql-Lit $CustomerCode), $(Sql-Lit $HostName), $(Sql-Lit $Name), $(Sql-Lit $Started.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss")), $(Sql-Lit $Finished.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss")), $ExitCode, $ok, $dur, $(Sql-Lit $msg), $(Sql-Lit $tail));
...
foreach ($j in $jobs) {
  ...
  Report-JobRun -Name $name -Started $started -Finished $finished -ExitCode $code -Message ("exit=" + $code) -LogTail $tail
