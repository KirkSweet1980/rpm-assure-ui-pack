# Vault Tech. Administrator on the customer host.
# Pack from Assure HTTPS only. No Git. No GitHub.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\Deploy-VAULT-Agent.ps1
param(
...
Invoke-WebRequest -UseBasicParsing -TimeoutSec 90 `
  ...
  -OutFile $deploy
& $deploy -CustomerCode 'VAULT' -RoleTags 'rmm' -AgentSecret $AgentSecret -AppHttpsUrl $AppHttpsUrl
