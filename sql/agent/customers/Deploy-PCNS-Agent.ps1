# BHF / PNCS. Administrator on the customer host.
# Pack from Assure HTTPS only. No Git. No GitHub.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\Deploy-PCNS-Agent.ps1
param(
  [string]$AgentSecret = '',
...
Invoke-WebRequest -UseBasicParsing -TimeoutSec 90 `
  ...
  -OutFile $deploy
& $deploy -CustomerCode 'PCNS' -RoleTags 'rmm' -AgentSecret $AgentSecret -AppHttpsUrl $AppHttpsUrl
