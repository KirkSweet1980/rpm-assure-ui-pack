# MEDiPOS Medical Scheme. Administrator on the customer host.
# Pack from Assure HTTPS only. No Git. No GitHub.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\Deploy-MEDIPOS-Agent.ps1
param(
...
Invoke-WebRequest -UseBasicParsing -TimeoutSec 90 `
  ...
  -OutFile $deploy
& $deploy -CustomerCode 'MEDIPOS' -RoleTags 'syspro' -AgentSecret $AgentSecret -AppHttpsUrl $AppHttpsUrl
