# RPM Resources. Administrator on the customer host.
# Pack from Assure HTTPS only. No Git. No GitHub.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\Deploy-RPMINT-Agent.ps1
param(
...
Invoke-WebRequest -UseBasicParsing -TimeoutSec 90 `
  ...
  -OutFile $deploy
& $deploy -CustomerCode 'RPMINT' -RoleTags 'syspro,sql' -AgentSecret $AgentSecret -AppHttpsUrl $AppHttpsUrl
