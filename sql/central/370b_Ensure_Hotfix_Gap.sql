/*
  CENTRAL 370b ? hotfix gap (no HotfixGuid required on Syspro_Hotfix)
*/
USE RPMAssure_App;
GO
SET NOCOUNT ON;
...
BEGIN
  ...
  (
    BaselineId       uniqueidentifier NOT NULL
      CONSTRAINT DF_Dim_HfBaseline_Id3 DEFAULT (NEWSEQUENTIALID()),
    ProductFamily    nvarchar(50) NOT NULL
      CONSTRAINT DF_Dim_HfBaseline_Fam3 DEFAULT (N'SYSPRO8'),
    ...
    Active           bit NOT NULL
      CONSTRAINT DF_Dim_HfBaseline_Active3 DEFAULT (1),
    ImportedAtUtc    datetime2(3) NOT NULL
      CONSTRAINT DF_Dim_HfBaseline_Imp3 DEFAULT (SYSUTCDATETIME()),
    CONSTRAINT PK_Dim_Syspro_HotfixBaseline3 PRIMARY KEY (BaselineId)
  );
...
BEGIN
  ...
  PRINT N'Dim_Syspro_HotfixBaseline OK';
END
GO
...
BEGIN
  ...
  (
    ...
    ImportedAt       datetime2(3) NOT NULL
      CONSTRAINT DF_Syspro_HfInst_Imp3 DEFAULT (SYSUTCDATETIME()),
    CONSTRAINT PK_Syspro_HotfixInstalled3 PRIMARY KEY (SnapshotDate, InstanceName, HotfixCode)
  );
...
BEGIN
  ...
  PRINT N'Syspro_HotfixInstalled OK';
END
GO

/* Mirror Syspro_Hotfix -> Installed WITHOUT HotfixGuid column dependency */
  ...
  BEGIN TRY
   ...
   AND t.HotfixCode = s.HotfixCode
    WHEN NOT MATCHED THEN
      INSERT (SnapshotDate, InstanceName, HotfixCode, Title, InstalledAt, Source)
      VALUES (s.SnapshotDate, s.InstanceName, s.HotfixCode, s.Title, s.InstalledAt, s.Source);
    PRINT CONCAT(N'Mirror Syspro_Hotfix -> Installed OK, @@ROWCOUNT=', @@ROWCOUNT);
  END TRY
  BEGIN CATCH
...
END
ELSE
  PRINT N'Syspro_Hotfix missing - skip mirror';
GO
