...
$CustomerCode = (Read-Default 'Customer code (2-20 A-Z0-9)' 'SIRF').ToUpperInvariant()
if ($CustomerCode -notmatch '^[A-Z0-9]{2,20}$') { throw 'CustomerCode must be 2-20 chars A-Z / 0-9.' }
$DisplayName  = Read-Default 'Display name' 'Sir Fruit'
$LocalServer  = Read-Default 'This SQL server (host or host,port)' $env:COMPUTERNAME
$InstanceName = Read-Default 'SYSPRO instance name (stored in Assure)' $LocalServer
