...
------------------
  ...
  Caddy renews automatically while it is running (~30 days before expiry).

  App HTTPS and agent HTTPS use THE SAME certificate
  (https://assure.rpmresources.co.za — /login, /downloads, /api/agent/*).

  Watchdog (SYSTEM, highest):
    RPMAssure-Https-Renew          daily 04:15
    RPMAssure-Https-Renew-PM       daily 16:15
    RPMAssure-Https-Renew-OnStart  reboot + 2 minutes
  Install / refresh:
  ...
  Status:
    C:\RPM-Assure\deploy\logs\https-renew-status.json

  Do not bind this cert to SQL Server. Agents talk HTTPS only.
